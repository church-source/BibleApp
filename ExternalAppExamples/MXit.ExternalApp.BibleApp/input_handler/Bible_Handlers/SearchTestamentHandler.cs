...
{
    class SearchTestamentHandler : AInputHandler
    {
           ...
           // Console.WriteLine("in input handler: " + input);
            ...
            foreach (MenuOptionItem option in options)
            {
                if (option.link_val.Equals(input))
                {
                    ...
                    user_session.setVariable(SEARCH_TESTAMENT_VAR_NAME, final_input.ToString());
        ...
        public const String SEARCH_TESTAMENT_VAR_NAME = "Search_Handler.testament";
    }
