...
namespace MxitTestApp
{
    public class BibleHelper
    {
        ...
        public static Dictionary<string, string> book_id_map = new Dictionary<string, string>();
        ...
        static BibleHelper()
        {
            ...
            try
            {
                ...
                sqlQuery = "SELECT id, name FROM books";
                ...
                {
                    id = (rdr[0]).ToString();
                    book_name = (rdr[1]).ToString();
                    book_id_map.Add(id, book_name);
                }
                rdr.Close();
                conn.Close();
            }
        ...
        public static String getVerseSectionReference(UserSession us, String sv, String ev)
        {
            Verse start_verse = Verse_Handler.getStartingVerse(us.user_profile.getDefaultTranslationId(), sv);
            if (start_verse != null)
            {
                Verse end_verse;
                if (ev == null || sv.Equals(ev))
                    end_verse = null;
                else if ("NULL".Equals(ev))
                    end_verse = BrowseBibleScreenOutputAdapter.getDefaultEndVerse(start_verse);
                else
                    end_verse = Verse_Handler.getStartingVerse(us.user_profile.getDefaultTranslationId(), ev);

                return  BibleHelper.getVerseSectionReferenceWithoutTranslation(start_verse, end_verse);
            }
            return "";
        }
