...
namespace MxitTestApp
{
    class ShortCodeOptionSet : AMenuDynamicOptionSet
    {
            ...
            if (book_list != null)
            {
                ...
                for (int i = 0; i < book_list.Count; i++)
                {
                    if (book_list[i].testament.testament_id
                        == Int32.Parse(us.getVariable("ShortCode_Handler.testament_id")))
                    {
                        final_list.Add(
                            new MenuOptionItem(
                                (book_list[i].name).ToString(),
                                (book_list[i].name).ToString(),
                                target_page,
                                book_list[i].name + " - " + BibleHelper.getShortCodeStringList(book_list[i].name)));
                    }
                }
                return final_list;
            } 
        ...
        public override string parseInput(String input, UserSession us)
        {
            ...
            string test_id = (String)us.getVariable("ShortCode_Handler.testament_id");
            if (test_id == "1")
                starting_index += 39;
